...
class AnsibleCLI:
            ...
            for pattern in ['*.yaml', '*.yml']:
                for pb_path in self.playbooks_dir.glob(pattern):
                    ...
                    playbooks[name] = {
                        ...
                        'description': config.get('playbooks', {}).get(name, {}).get('description', ''),
                        'tags': config.get('playbooks', {}).get(name, {}).get('tags', []),
                        'requires': config.get('playbooks', {}).get(name, {}).get('requires', []),
                        'order': config.get('playbooks', {}).get(name, {}).get('order', 999)
                    }
    ...
    def list_playbooks(self, verbose: bool = False):
        ...
        if not self.playbooks:
            print(f"{Colors.WARNING}Aucun playbook trouvé dans {self.playbooks_dir}{Colors.ENDC}")
            return
    ...
    def duplicate_example_files(self):
        ...
        for example_file in self.base_dir.rglob("example.main.yaml"):
            ...
            if not target.exists():
                ...
                print(f"{Colors.OKGREEN}✓{Colors.ENDC} Créé: {target.relative_to(self.base_dir)}")
                count += 1
            else:
                print(f"{Colors.WARNING}→{Colors.ENDC} Existe déjà: {target.relative_to(self.base_dir)}")
    ...
    def _run_playbook(
        ...
        print(f"{Colors.OKCYAN}[{start_time.strftime('%H:%M:%S')}] Démarrage: {playbook_name}{Colors.ENDC}")
        ...
        try:
            ...
            if result.returncode == 0:
                print(f"{Colors.OKGREEN}[{end_time.strftime('%H:%M:%S')}] ✓ Succès: {playbook_name} ({duration:.1f}s){Colors.ENDC}")
            else:
                print(f"{Colors.FAIL}[{end_time.strftime('%H:%M:%S')}] ✗ Échec: {playbook_name} ({duration:.1f}s){Colors.ENDC}")
            
            return (playbook_name, result.returncode, result.stdout, result.stderr)
            
        except Exception as e:
            end_time = datetime.now()
            print(f"{Colors.FAIL}[{end_time.strftime('%H:%M:%S')}] ✗ Erreur: {playbook_name} - {str(e)}{Colors.ENDC}")
    ...
    def run_playbooks(
        ...
        **kwargs
        ...
        if parallel and len(ordered_playbooks) > 1:
            print(f"{Colors.BOLD}Mode parallèle activé (max {max_workers} workers){Colors.ENDC}\n")
    ...
    def print_summary(self, results: List[Tuple[str, int, str, str]]):
        ...
        for name, return_code, stdout, stderr in results:
            status = f"{Colors.OKGREEN}✓ SUCCÈS{Colors.ENDC}" if return_code == 0 else f"{Colors.FAIL}✗ ÉCHEC{Colors.ENDC}"
            print(f"  {status} - {name}")
...
@click.option('-e', '--extra-vars', type=str, help='Variables supplémentaires (format JSON)')
...
@click.option('--max-workers', type=int, default=4, help='Nombre de workers parallèles (défaut: 4)')
...
@pass_cli
def run(cli_obj, playbooks, all, inventory, extra_vars, tags, skip_tags, 
        parallel, max_workers, dry_run, verbose, show_output):
    """Exécute un ou plusieurs playbooks.
    ...
    if not playbooks and not all:
        click.secho('Erreur: Spécifiez des playbooks ou utilisez --all', fg='red', err=True)
        raise click.Abort()
    ...
    if invalid:
        click.secho(f"Erreur: Playbooks non trouvés: {', '.join(invalid)}", fg='red', err=True)
    ...
    if extra_vars:
        try:
            ...
            click.secho(f"Erreur: Format JSON invalide pour --extra-vars: {e}", fg='red', err=True)
            raise click.Abort()
